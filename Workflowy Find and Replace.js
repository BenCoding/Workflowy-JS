(function FR_1_5() {
  ...
  function getItemsWithVisibleMatches(parent) {
    return findMatchingItems(function (item) {
      let itemTree = item.getProjectTreeObject();
      if (itemTree) {
        let isVisible = WF.completedVisible() || !item.isCompleted();
        return itemTree.search_result && itemTree.search_result.matches && isVisible
      }
    }, parent)
  }
  ...
  function countMatches(items, rgx) {
    let matchCount = 0;
    items.forEach(item => {
      let result = item.getProjectTreeObject().search_result;
  ...
  if (!WF.currentSearchQuery()) {
    return void toastMsg('Use the searchbox to find. <a href="https://workflowy.com/s/C7C.M60IuWhoJq" target="_blank">Click here for more information.</a>', 3, true);
  }
  const tQuery = WF.currentSearchQuery().trim();
  const Matches = getItemsWithVisibleMatches(WF.currentItem());
